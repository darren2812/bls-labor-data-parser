...
void DynamicArray::readEntries(std::ifstream& rawData, Occupation* allJobs, const std::string* headings, int* columnLengths, int& jobCounter) {
	...
	*/
	...
	while (std::getline(rawData, tempString)) {
		switch (columnCount) {
		case 0:
			allJobs[jobCounter].setOccupation(tempString);
			break;
		case 1:
			allJobs[jobCounter].setMatrixCode(tempString);
			break;
		case 2:
			allJobs[jobCounter].setOccupationType(tempString);
			break;
		case 3:
			allJobs[jobCounter].setEmploymentCurrent(tempString);
			break;
		case 4:
			allJobs[jobCounter].setEmploymentFuture(tempString);
			break;
		case 5:
			allJobs[jobCounter].setDistributionCurrent(tempString);
			break;
		case 6:
			allJobs[jobCounter].setDistributionFuture(tempString);
			break;
		case 7:
			allJobs[jobCounter].setNumericChange(tempString);
			break;
		case 8:
			allJobs[jobCounter].setPercentageChange(tempString);
			break;
		case 9:
			allJobs[jobCounter].setPercentSelfEmployed(tempString);
			break;
		case 10:
			allJobs[jobCounter].setJobOpenings(tempString);
			break;
		case 11:
			allJobs[jobCounter].setWage(tempString);
			break;
		case 12:
			allJobs[jobCounter].setEducation(tempString);
			break;
		case 13:
			allJobs[jobCounter].setWorkExperience(tempString);
			break;
		case 14:
			allJobs[jobCounter].setTraining(tempString);
			break;
		case 15:
			allJobs[jobCounter].setHandbookContent(tempString);
			allJobs[jobCounter].setJobIndex(jobCounter);
			columnCount = -1;
			jobCounter++;
			break;
		default:
...
Occupation DynamicArray::addEntry(Occupation*& allJobs, int& jobCounter, int& totalJobsCapacity, std::string& jobAdded, int* columnLengths, std::string key) {
	...
	if (totalJobsCapacity - jobCounter < 1) {
		totalJobsCapacity++;
	...
	case 'A': // assigns "Summary" occupation type
		allJobs[jobCounter].setOccupationType("Summary");
		break;
	case 'B': // assigns "Line item" occupation type
		allJobs[jobCounter].setOccupationType("Line item");
		break;
	default:
		break;
	}
	...
	while (g_catch == true || tempFloat < 0.0f) {
		// passes "-" to setter
		if (tempString == "-") {
			allJobs[jobCounter].setEmploymentCurrent("-");
			break;
		}
	...
	while (g_catch == true || tempFloat < 0.0f) {
		// passes "-" to setter
		if (tempString == "-") {
			allJobs[jobCounter].setEmploymentFuture("-");
			allJobs[jobCounter].setNumericChange("-");
			allJobs[jobCounter].setPercentageChange("-");
			break;
		}
		// asks user for input if they inputted characters or negative values
		else {
	...
	if (tempString != "-") {
		...
		tempFloat = allJobs[jobCounter].getEmploymentFuture() - allJobs[jobCounter].getEmploymentCurrent();
		...
		tempFloat = ((allJobs[jobCounter].getEmploymentFuture() - allJobs[jobCounter].getEmploymentCurrent()) / allJobs[jobCounter].getEmploymentCurrent())
			* 100;
		roundedString << std::fixed << std::setprecision(1) << round(tempFloat * 10) / 10.0f;
		tempString = roundedString.str();
		allJobs[jobCounter].setPercentageChange(tempString);
	...
	switch (tempString[0]) {
	case 'A':
		allJobs[jobCounter].setEducation("No formal educational credential");
		break;
	case 'B':
		allJobs[jobCounter].setEducation("High school diploma or equivalent");
		break;
	case 'C':
		allJobs[jobCounter].setEducation("Postsecondary nondegree award");
		break;
	case 'D':
		allJobs[jobCounter].setEducation("Some college, no degree");
		break;
	case 'E':
		allJobs[jobCounter].setEducation("Associate�s degree");
		break;
	case 'F':
		allJobs[jobCounter].setEducation("Bachelor�s degree");
		break;
	case 'G':
		allJobs[jobCounter].setEducation("Master�s degree");
		break;
	case 'H':
		allJobs[jobCounter].setEducation("Doctoral or professional degree");
		break;
	case '-':
		allJobs[jobCounter].setEducation("-");
		break;
	default:
		break;
	}
	...
	switch (tempString[0]) {
	case 'A':
		allJobs[jobCounter].setWorkExperience("None");
		break;
	case 'B':
		allJobs[jobCounter].setWorkExperience("Less than 5 years");
		break;
	case 'C':
		allJobs[jobCounter].setWorkExperience("5 years or more");
		break;
	case '-':
		allJobs[jobCounter].setWorkExperience("-");
		break;
	default:
		break;
	}
	...
	switch (tempString[0]) {
	case 'A':
		allJobs[jobCounter].setTraining("None");
		break;
	case 'B':
		allJobs[jobCounter].setTraining("Short-term on-the-job training");
		break;
	case 'C':
		allJobs[jobCounter].setTraining("Moderate-term on-the-job training");
		break;
	case 'D':
		allJobs[jobCounter].setTraining("Long-term on-the-job training");
		break;
	case 'E':
		allJobs[jobCounter].setTraining("Apprenticeship");
		break;
	case 'F':
		allJobs[jobCounter].setTraining("Internship/residency");
		break;
	case '-':
		allJobs[jobCounter].setTraining("-");
		break;
	default:
		break;
	}
